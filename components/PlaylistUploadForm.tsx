...
export default function PlaylistUploadForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!title || !url || !thumbnailUrl || !chapterId) {
    ...
    try {
      ...
      toast.error('Failed to submit playlist');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        <label htmlFor="thumbnailUrl" className="block text-sm font-medium text-gray-200 mb-2">
          Thumbnail URL
        </label>
        <input
          id="thumbnailUrl"
